...
def get_Datasets():
    ...
    return np.concatenate((dataSet, classLabels.reshape((-1, 1))), axis=1)  # (200, 100+1)
...
def select_best_feature(dataSet, m, alpha="huigui"):
    ...
    if alpha == "huigui":
        ...
        S = gini(dataSet)  # 计算数据集真实Gini值
    for i in range(m):
        index.append(np.random.randint(f))  # 随机挑m个属性
    for feature in index:
        for splitVal in set(dataSet[:, feature]):  # 该feature有哪些值
    ...
    if (S-bestS) < 0.001 and alpha == "huigui":    # 如果误差不大就退出
        ...
        return None, MostNumber(dataSet)  # 返回样本中最多的那个label
        # print(S, bestS)
...
if __name__ == '__main__':
    ...
    treeNumber = 4
    RomdomTrees = RondomForest(dataSet, treeNumber, alpha="fenlei")   # 4棵树，分类。
    print("---------------------RomdomForest------------------------")
    for i in range(treeNumber):
        print("tree", i+1, "：", RomdomTrees[i])
    yhat = predictTree(RomdomTrees, dataSet, alpha="fenlei")
    print("---------------------Forecast Resultes------------------------")
    print(yhat.T)
